...
# ====================================================================================================================================================================================================
    ...
    if filterMode == "Username" or filterMode == "Text" or filterMode == "NameAndText":
      ...
      while validFilterSubMode == False:
        ...
        if filterSubMode in validFilterSubModes:
          ...
          print(f"\nInvalid choice: {filterSubMode} - Enter 1 or 2:")
          validConfigSetting = False
    ...
    if not current.matchedCommentsDict and not current.duplicateCommentsDict and not current.spamThreadsDict and not current.repostedCommentsDict: # If no spam comments found, exits
      print(f" No matched comments or users found! \n")
    ...
    if bypass == False:
      ...
      print("\nComments ready to display. Also save a log file?")
      print(f"(It even allows you to restore deleted comments later)")
      loggingEnabled = choice(f"Save Log File?")
    ...
    if confirmDelete == None and returnToMenu == False:
      ...
      while confirmDelete == None or confirmDelete.lower() not in validResponses:
        ...
        if current.errorOccurred == True:
          print("\n--- NOTE: Options limited due to error during scanning ---")
        if exclude == False:
          print("How do you want to handle all the listed comments above?")
        elif exclude == True:
          print("How do you want to handle the rest of the comments (not ones you excluded)?")
        if userNotChannelOwner == True and moderator_mode == False:
        ...
        if exclude == False:
          if userNotChannelOwner == False and current.errorOccurred == False:
            print(f" > To Delete all of the above comments: Type '{F.LIGHTRED_EX}DELETE{S.R}', then hit Enter.")
          if (userNotChannelOwner == False or moderator_mode == True) and current.errorOccurred == False:
            print(f" > To Move all comments above to 'Held For Review' in YT Studio: Type '{F.LIGHTRED_EX}HOLD{S.R}', then hit Enter.")
        elif exclude == True:
          if userNotChannelOwner == False and current.errorOccurred == False:
            print(f" > To Delete the rest of the comments: Type '{F.LIGHTRED_EX}DELETE{S.R}', then hit Enter.")
          if (userNotChannelOwner == False or moderator_mode == True) and current.errorOccurred == False:
            print(f" > To Move rest of comments above to 'Held For Review' in YT Studio: Type '{F.LIGHTRED_EX}HOLD{S.R}', then hit Enter.")

        # Report & None
        if current.errorOccurred == False:
          print(f" > To report the comments for spam, type '{F.LIGHTCYAN_EX}REPORT{S.R}'.")
        if loggingEnabled:
          print(f" > To do nothing and only log, type '{F.YELLOW}NONE{S.R}'")
        else:
          print(f" > To do nothing, type '{F.YELLOW}NONE{S.R}'")
...
# -------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  try:
    ...
    if "referenced before assignment" in str(ux):
      print(f"\n{F.LIGHTRED_EX}Error - Code: X-2{S.R} occurred.")

      input("\n Press Enter to Exit...")
    else:
      traceback.print_exc()
      print("------------------------------------------------")
      print(f"\n{F.LIGHTRED_EX}Unknown Error - Code: Z-2{S.R} occurred")
    ...
    if "config" in str(kx):
      ...
      print(f"{F.RED}Unknown Error - Code: X-4{S.R} occurred")
    ...
    print(f"\n{F.LIGHTRED_EX}Unknown Error - Code: Z-3{S.R} occurred.")
