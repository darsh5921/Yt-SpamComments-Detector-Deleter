...
def get_authenticated_service():
  ...
  if not os.path.exists(CLIENT_SECRETS_FILE):
    ...
    if os.path.exists(f"{CLIENT_SECRETS_FILE}.json"):
      ...
      print(f"\n--------> {F.RED}[!] Error:{S.R} client_secrets.json file not found <--------")
      print(f"-----> Did you create a {F.YELLOW}Google Cloud Platform Project{S.R} to access the API? <----- ")
      print("\n# Create the file and rename it to client_secrets for program to work (Place it in installation folder of the Program)")
      input("\nPress Enter to Exit...")
      sys.exit()
...
def get_current_user(config):
  ...
  try:
    ...
    try:
      ...
      print("Error Getting Current User: Channel ID was found, but channel title was not retrieved. If this occurs again, try deleting 'token.pickle' file and re-running.")
    ...
    traceback.print_exc()
    print("\nError: Still unable to get channel info. Try deleting token.pickle.")
