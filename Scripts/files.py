      ...
      if userChoice == True:
        if sys.platform == 'win32' or sys.platform == 'win64':
          ...
          if os.stat(downloadFileName).st_size == total_size_in_bytes:
            ...
            print("Something is wrong with the download on the remote end. You should manually download latest version")
    ...
    if updating == False and dontWarn == False:
      ...
      if userChoice.lower() == "x":
        ...
        try:
          ...
          traceback.print_exc()
          print("Error Code F-1: Problem deleting existing file.")
...
def parse_comment_list(config, recovery=False, removal=False, returnFileName=False):
  ...
  while validFile == False and manuallyEnter == False:
    print("--------------------------------------------------------------------------------")
    print(f"\nEnter the name of the log file with comments to {actionVerb} (you can rename it to something easier like \'log.rtf\')")
    print(f">TIP: You can just drag the file into this window instead of typing it or enter it manually from installed folder location")
    ...
    if len(listFileName) > 0:
      ...
      if os.path.exists(listFileName):
        ...
        print(f"\n{F.RED}Error: File not found.{S.R} Make sure it is in the same folder as the program.\n")
        print(f"Enter '{F.WHITE}Y{S.R}' to try again")
